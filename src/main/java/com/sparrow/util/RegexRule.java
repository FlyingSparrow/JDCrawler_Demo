...
public class RegexRule {
	...
	/**
	 * 添加正则规则 正则规则有两种，正正则和反正URL符合正则规则满足下面条件1.至少能匹配一条正正则 2.不能和任何反正则匹配
	 *
	 * @param rule
	 *            正则规律，需要按照正反正则的规范输入，在普通正则前加入一个正(号来表示正反正则（正正则可不加符号，或??+",反正则必须加?-')
	 */
	...
	/**
	 * 获取下一个符合正则规则的爬取任务 URL符合正则规则?满足下面条件?1.至少能匹配一条正正则 2.不能和任何反正则匹配
	 *
